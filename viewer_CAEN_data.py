...
#fig1.savefig('Inverter2_900_impulses.png', dpi=600)
plt.show()
...
for board_idx in boards:
    ...
    for i in range(len(control_timeline)):
        control_timeline[i] = control_timeline[i] / len(data)
    ...
    #for ch in [3, 4]:
        ...
        #ax2.set_xlim(-15, 20)
        with open('reference_signal.txt', 'w') as file:
            for item in range(len(sum_waveform[board_idx][ch])):
                print(control_timeline[item],',', sum_waveform[board_idx][ch][item], file=file)



#fig2.savefig('Inverter2_averaged.png', dpi=600)

plt.show()

'''with open('Splitter2_parametrs.txt', 'w') as file:
